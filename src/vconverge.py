...
#	prior_vars_cols = []
	...
	for i in range(len(linesog)):
		if linesog[i].split() == []:
			pass
		elif linesog[i].split()[0] == 'srcfolder': # Code will want to know source folder
		#elif linesog[i].split()[0] == 'sSrcFolder': ##################################################################################################################
		...
		elif linesog[i].split()[0] == 'destfolder': # Code will want to know dest folder
		#elif linesog[i].split()[0] == 'sDestFolder': ##################################################################################################################
			dest_fold = linesog[i].split()[1]
		elif linesog[i].split()[0] == 'samplemode': # If sample mode isn't random lets stop now
		#elif linesog[i].split()[0] == 'sSampleMode': ##################################################################################################################
		...
		elif linesog[i].split()[0] == 'randsize':
		#elif linesog[i].split()[0] == 'iRandSize': ##################################################################################################################
			initial_sim_size = int(linesog[i].split()[1])
		elif linesog[i].split()[0] == 'trialname':
		#elif linesog[i].split()[0] == 'sTrialName': ##################################################################################################################
			triname = linesog[i].split()[1]
		elif linesog[i].split()[0] == 'file' or linesog[i].split()[0] == 'sPrimaryFile': # Set the current file in case we need to append var names for predefined prior mode
		#elif linesog[i].split()[0] == 'sBodyFile' or linesog[i].split()[0] == 'sPrimaryFile': ##################################################################################################################
...
#			if values[2][0] == 'p':
#				if [values[0], values[1]] not in prior_files:
#					prior_files.append([values[0], values[1]])
#					priorvarhold = []
#					rand_var = curr_fi+'/'+var
#					priorvarhold.append(rand_var)
#					prior_vars.append(priorvarhold)
#					priorcolhold = []
#					priorcolhold.append(int(values[3])-1)
#					prior_vars_cols.append(priorcolhold)
#					
#				else:
#					for i in range(len(prior_files)):
#						if prior_files[i] == [values[0], values[1]]:
#							prior_index = i
#					rand_var = curr_fi+'/'+var
#					prior_vars[prior_index].append(rand_var)
#					prior_vars_cols[prior_index].append(int(values[3])-1)
#	if prior_files != []:
...
def create_tmp_vspin(vspFile, RunIndex, stepsize): # Creates a temporary vspace.in file to run for steps subsequent to original run
	...
	for i in range(len(linesog)):
		if linesog[i].split() == []:
			pass
		elif linesog[i].split()[0] == 'destfolder': # Destination folder needs to be in vconverge_tmp, unique runs will be copied into OG destfolder
		#elif linesog[i].split()[0] == 'sDestFolder': ##################################################################################################################
			vsptmp.write('destfolder vconverge_tmp/Step_'+str(RunIndex)+'\n')
		elif linesog[i].split()[0] == 'trialname': # Trial name must be unique to not overwrite any data during copy
		#elif linesog[i].split()[0] == 'sTrialName': ##################################################################################################################
...
#	bp.run('vspace.in') # example of how to call in python
