...
def ComputeAgeDistribution(daA, daB, daC, daD, daRotationPeriod, iNumSamples=100000):
    ...
    daFrequency = daASamples * daRotPerSamples + daBSamples + daCSamples * (daRotPerSamples - daDSamples)
    
    #print(f"Total samples generated: {len(daFrequency):,}")

    dMeanAge = np.mean(daFrequency)
    dStdDevAge = np.std(daFrequency)
    daAge95ConfidenceInterval = np.percentile(daFrequency, [2.5, 97.5])
    
    return daFrequency, dMeanAge, dStdDevAge, daAge95ConfidenceInterval
...
if __name__ == "__main__":
    ...
    daFrequency, dMeanAge, dStdDevAge, daAge95ConfidenceInterval = ComputeAgeDistribution(
        daA, daB, daC, daD, daRotationPeriod
    )
    
    dMeanAgeAnalytic = AnalyticalSolution(daA, daB, daC, daD, daRotationPeriod)
    
    print(f"Monte Carlo Results ({len(daFrequency):,} samples):")
    ...
    print(f"Range of plausible values: [{np.min(daFrequency):.4f}, {np.max(daFrequency):.4f}] (all ≤ 1.08)")
    ...
    print(f"\nAdditional Statistics:")
    print(f"Median: {np.median(daFrequency):.4f}")
    print(f"Mode (approximate): {stats.mode(np.round(daFrequency, 2))[0][0]:.2f}")
    print(f"Skewness: {stats.skew(daFrequency):.4f}")
    print(f"Kurtosis: {stats.kurtosis(daFrequency):.4f}")
    
    # Additional check for maximum constraint
    dMaxAge = np.max(daFrequency)
    print(f"\nConstraint Check:")
    print(f"Maximum age value (after filtering): {dMaxAge:.4f}")
    
    print(f"\nFull Range of Valid age Values:")
    print(f"Minimum age: {np.min(daFrequency):.4f}")
    print(f"Maximumage: {dMaxAge:.4f}")
    dShapiroStatistic, dShapiroP = stats.shapiro(daFrequency[:5000])  # Use subset for efficiency
    print(f"Shapiro-Wilk test p-value: {dShapiroP:.6f}")
    print(f"Is Age normally distributed? {'Yes' if dShapiroP > 0.05 else 'No'}")
    
    NormalizedHistogram(daFrequency,'k',label='')
    #plot_results(y_samples, y_mean, y_std, y_ci)
    
    sOutFile='age_samples.txt'
    np.savetxt(sOutFile, daFrequency, 
               header=f'Age samples og GJ 1132 for Engle (2024) model\nMean: {dMeanAge:.4f}, Std: {dStdDevAge:.4f}')
